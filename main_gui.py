...
# Narrator-bleed filter (shared, reset per-request in flush_think)
_narrator_stream = _StreamingNarratorFilter()
...
def on_chunk(chunk):
    for text, is_think in state.think_parser.feed(chunk):
        ...
        if is_think:
            ...
            # Filter narrator bleed from visible answer stream
            filtered = _narrator_stream.feed(text)
            if filtered:
                state.output_queue.put({"type":"answer","content":filtered})

def flush_think():
    # Flush and reset the streaming narrator filter
    leftover = _narrator_stream.flush()
    if leftover:
        state.output_queue.put({"type":"answer","content":leftover})
    _narrator_stream.reset()
...
def _build_tools(cwd):
    return [BashTool(working_dir=cwd), ReadFileTool(working_dir=cwd),
            ...
            PMTool(), ScratchpadTool(), ReflectTool(),
...
function delC(e,id){
  e.stopPropagation();
  fetch('/api/del_conv',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id})});
...
function send(){
  ...
  if(c) c.messages.push({role:'user',content:txt});
...
function handleSSE(ev){
  ...
  if(d.type==='tool_start'){streamTool(d.name,d.summary,++_tC);_pendId=_tC;}
    ...
    const c=convs.find(x=>x.id===activeId);
    if(c&&_ans) c.messages.push({role:'assistant',answer:_ans,tools:[]});
    finishStream();setBusy(false);es.close();updateStats();
  }
...
// pill helpers
function pillName(){return document.querySelector('.inp-footer .pill-name')?document.querySelector('.inp-footer .pill-name').textContent:'';}
function pillSize(){return document.querySelector('.inp-footer .pill-size')?document.querySelector('.inp-footer .pill-size').textContent:'';}
function pillQuant(){return document.querySelector('.inp-footer .pill-quant')?document.querySelector('.inp-footer .pill-quant').textContent:'';}
function appendMeta(w,d){
  const meta=mk('div','msg-meta');
  const badge=mk('div','meta-badge');
        ...
        def run():
            try:
                ...
                if should_ultraplan(message):
                    try:
                        up = UltraPlan(
                            tools=list(state.engine.tools.values()),
                            working_dir=state.engine.working_dir,
                        )
                        up.run(message, engine=state.engine)
                    except Exception:
...
@app.route('/api/del_conv', methods=['POST'])
def del_conv():
    cid = request.json.get('id')
    state.conversations = [c for c in state.conversations if c['id'] != cid]
    if state.active_conv_id == cid:
        if state.conversations:
            state.active_conv_id = state.conversations[-1]['id']
        else:
