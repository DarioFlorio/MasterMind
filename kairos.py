"""
kairos.py — Kairos: persistent background daemon (MasterMind Harness feature).
...
coordinate with the main MasterMind process.
...
def _tick(mem_dir):
    ...
    if last_push:
        try:
            ...
            if idle_h > 8:
                insights.append(f"⏰ MasterMind has been idle for {idle_h:.1f}h. Last summary: {context.get('last_summary','(none)')[:60]}")
        except Exception:
            pass
