"""
memory/autodream.py — AutoDream: idle-time memory consolidation (MasterMind Harness feature).

AutoDream runs as a background daemon thread. When MasterMind has been idle for
...
class AutoDream:
    ...
    def _watch_loop(self) -> None:
        # Import consolidation lock for cross-process coordination
        try:
        ...
        while not self._stop_evt.wait(timeout=15):
            ...
            # Also respect cross-process lock mtime as cooldown
            if _lock_available and cd_ok:
                last_consolidated = read_last_consolidated_at(self._mem_dir)
                if last_consolidated > 0:
                    cross_proc_age = time.time() - last_consolidated
                    if cross_proc_age < DREAM_COOLDOWN_S:
                        cd_ok = False
                        log.debug(
                            "AutoDream: another process consolidated %.0fs ago — skipping",
                            cross_proc_age
                        )
            ...
            if idle >= IDLE_TRIGGER_S and cd_ok and not self._dreaming:
                ...
                # Try to acquire cross-process lock before dreaming
                prior_mtime = None
                if _lock_available:
                    prior_mtime = try_acquire_consolidation_lock(self._mem_dir)
                    if prior_mtime is None:
                        log.debug("AutoDream: lock held by another process — skipping this cycle")
                        continue
                ...
                try:
                    ...
                    if _lock_available:
                        record_consolidation(self._mem_dir)
                    log.info("AutoDream: dream complete — %s", summary[:120])
                except Exception as e:
                    log.error("AutoDream: dream cycle error: %s", e)
                    if _lock_available and prior_mtime is not None:
                        rollback_consolidation_lock(self._mem_dir, prior_mtime)
                finally:
                    self._dreaming = False
